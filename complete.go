...
func APKHunt_core(apkpath string) {
	...
	for _, sources_file := range files {
		...
		if ext == ".java" || ext == ".kt" {
			cmd_and_pkg_unencryptedProtocol, err := exec.Command("grep", "-nri", "-e", "http://", "-e", "http:", "-e", "SSLCertificateSocketFactory.getInsecure(", sources_file).CombinedOutput()
			if err != nil {
			...
			if strings.Contains(cmd_and_pkg_unencryptedProtocol_output, "http://") || strings.Contains(cmd_and_pkg_unencryptedProtocol_output, "http:") || strings.Contains(cmd_and_pkg_unencryptedProtocol_output, "getInsecure") {
				fmt.Printf(string(colorBrown))
				log.Println(sources_file)
				fmt.Printf(string(colorReset))
